...
- `game.js`: Contains all the game logic written in JavaScript.

### Key Functions

- `generateRandomNumber()`: Generates a random number between 1 and 100.
- `getPlayerGuess(message)`: Handles user input via prompts, validates it, and allows escaping.
- `checkGuess(guess, correctNumber)`: Compares the guess to the correct number and returns feedback.
- `game()`: The main game loop that manages attempts, prompts, and win/loss conditions.

The game uses browser prompts for input and console logs for output, creating an interactive experience through the developer tools.
